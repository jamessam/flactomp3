...
def get_valid_path(resolution):
    while True:
        path = input(f'The path of the {resolution}-resolution files: ')
...
def make_mp3s(high_res_path, low_res_path, file_list, extension):
    for f in file_list:
        ...
        command = make_command(high_res_path, low_res_path, f.strip(), extension.strip())
...
def make_command(high_res_path, low_res_path, master_file, extension):
    ...
    command = ['ffmpeg', '-i', full_master, '-write_id3v1', '1','-id3v2_version', '3',
        '-dither_method','modified_e_weighted', '-out_sample_rate','48k', '-b:a','320k', full_mp3]
